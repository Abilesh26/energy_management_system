...
char ssid[] = "Abi's Nord";   // your network SSID (name) 
char pass[] = "abi@7177";   // your network password
...
void loop() {
int co = ThingSpeak.readIntField(596974,1,"NRLP8LEVYXY21JMC");  
if(co==2)
{
  digitalWrite(D4,LOW);
  }
  if(co==1)
{
  ...
  float acVoltageRMS = calculateACVoltageRMS(1);

  // Read AC current from channel 1
  float acCurrentRMS = calculateACCurrentRMS(2);
...
un=un+(acVoltageRMS*((acCurrentRMS/1000))/1000);
ThingSpeak.setField(1,acVoltageRMS);
  ThingSpeak.setField(2, acCurrentRMS);
  ThingSpeak.setField(3, un/1000);
